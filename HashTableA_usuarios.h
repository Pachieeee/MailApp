...
public:
	...
	{
		auto& whichList = theLists[myhash(x.getPais())]; //Del vector, obtenemos la lista de elementos seg̼n el hash(indice) obtenido
	...
	void DispAll() {
		...
		for (auto& thisList : theLists) {		// Recorremos el vector<>
			...
			for (auto& it : theLists[pos]) {	// Recorremos la Lista de cada indice del vector	
				// cout << it.getPais() << ",";		// Imprime key
				cout << "(" << it.getPais() << " - " << it.getCargo() <<" " << it.getApellido() << "); ";	// Imprime (key,value)
			}
			cout << endl;
			pos++;
		}
	...
	void buscar(string key) {
		...
		for (auto& it : theLists[pos]) {	// Recorremos la Lista de cada indice del vector	
			if (it.getPais() == key)
				cout << "(" << it.getPais() << ", " << it.getCargo() << ", " << it.getApellido() << ", " << it.getCorreo() << "); ";	// Imprime (key,values)
		}
		cout << endl;
	}
